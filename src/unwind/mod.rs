...
impl UnwindContext {
    ...
    pub(crate) fn emit(self, product: &mut ObjectProduct) {
        let mut eh_frame = EhFrame::from(emit::WriterRelocate::new(self.endian));
    ...
    pub(crate) unsafe fn register_jit(self, jit_module: &crate::jit::JITModule) {
...
unsafe extern "C" {
    // libunwind import
    unsafe fn __register_frame(fde: *const u8);
}
